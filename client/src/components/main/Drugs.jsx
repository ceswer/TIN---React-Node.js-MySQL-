...
export default function Doctors() {
    ...
    const changeToTrue = async () => {
        setShowButton(false);
        await fetch(`http://localhost:3000/api/drugs/${ids}`,
            {
                method: 'DELETE',
                headers: {
    ...
    const getDoctors = async () => {
        await fetch("http://localhost:3000/api/drugs", { headers: { authorization: `Bearer ${localStorage.getItem('token')}`}})
            .then(res => res.json())
            .then(docs => {
                setDoctors(docs)
                setIsLoaded(true);
    ...
    return (
        <main>
            ...
            <div className="div-button-add">
                {localStorage.getItem('user') !== 'user' ? <Link to="/drugs/add" className="button-add">{t('add')}</Link> : ""}
            </div>
        </main>
    );
}
        ...
        <tr key={doc.id}>
            ...
            <td>
                <ul className="list-actions">
                    <li><Link to={"/drugs/details/" + doc.id} className="list-actions-button-details">{t('about')}</Link></li>
                    {localStorage.getItem('user') !== 'user' ? <li><Link to={"/drugs/edit/" + doc.id} className="list-actions-button-edit">{t('edit')}</Link></li> : "" }
                    {localStorage.getItem('user') !== 'user' ? <li><button onClick={() => deleteDoctor(doc.id)} className="list-actions-button-delete">{t('delete')}</button></li> : ""}
                </ul>
            </td>
        </tr>
    );
